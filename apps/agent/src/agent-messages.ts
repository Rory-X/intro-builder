...
export function buildAgentMessagePrompt(
  ...
  return {
    ...
    developer: [
      ...
      "JSON schema:",
      '{"message":{"id":"string","role":"assistant","content":"string"},"toolCalls":[{"id":"string","name":"resume_read|get_completeness|set_goal|role_match_read|ats_check|content_claim_audit|layout_fit_check|section_quality_score|resume_update_section|resume_delete_section|resume_reorder_sections|resume_insert_section|resume_polish_text|resume_set_text|resume_ask","status":"completed","title":"string","summary":"string","input":{},"result":{}}],"proposedOperations":[{"id":"string","toolCallId":"必须等于某个 toolCalls[].id","label":"string","section":"summary|experience|projects|education|skills|research|custom","fieldPath":"string","operation":"update_section|delete_section|reorder_sections|insert_section","beforePlainText":"string","afterPlainText":"string","replacementTiptapJson":{},"sectionOrder":["string"],"changeSummary":"string","riskFlags":[{"type":"needs_user_fact|possible_fabrication|formatting_risk|unsafe_claim","message":"string"}]}],"questions":[{"id":"string","message":"string","field":"可选，例如 goal.targetRole","responseSchema":{}}],"draftResume":{"title":"string","targetRole":"string|null","profileSummary":"string","sections":[{"key":"string","label":"string","summary":"string","status":"drafted|needs_user_fact"}],"missingFacts":["string"]}}',
      "可用 internal loop tools: resume_read, get_completeness, set_goal, role_match_read, ats_check, content_claim_audit, layout_fit_check, section_quality_score, resume_polish_text, resume_set_text, resume_update_section, resume_delete_section, resume_reorder_sections, resume_insert_section, resume_ask",

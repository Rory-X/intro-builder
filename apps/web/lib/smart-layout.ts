...
// 1.05 行距是挤成一团的丑态，抬高后压缩永远落在好看的区间里。
// pagePadding 不参与压缩——页边距是品牌/视觉决策（zoo 反馈：智能排版不能影响
// 页边距）。headingGap 是标题到正文的模板内间距，也不参与压缩；"标题间距"
// 控件现在调 sectionGap（不同 section 之间的距离）。fontSize 作为最后兜底分级
// 下降到 MIN_FONT，但只影响正文；个人信息栏字号独立保护（通过 --profile-font-size
// CSS 变量，在 measure 时锁定原值）。
const MIN_FONT = 11;
 ...
 * 反向把它放大回 floor。fontFamily / headingGap / pagePadding / photoScale 始终保持 current。
 */
